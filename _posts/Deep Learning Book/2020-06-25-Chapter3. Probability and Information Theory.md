...
<h2>3.13. Information Theory</h2>

정보 이론은 어떤 사건, 신호가 어느 정도의 정보를 가지고 있는지 정량화 한다.
이는 흔하게 발생하는 사건보다, 가끔 발생하는 사건이 더 많은 정보량을 가지고 있다는 것에서 출발한다.
"오늘 아침에 해가 떳다" 보다 "오늘 아침에 일식이 있었다"가 더 많은 정보를 가지고 있다는 것이다.

- 흔하게 발생하는 사건은 적은 정보량을 가진다. 극단적으로, 항상 발생하는 것이 보장된 사건은 정보량이 없어야 한다.
- 흔하지 않은 사건은 상대적으로 높은 정보량을 가진다.
- 독립사건은 추가적인 정보를 가진다.

이를 따라서, $$x=\mathrm{x}$$인 사건에 대한 self-information은 다음과 같이 정의된다.

$$I(x)=- \log P(x). \tag{3.48}$$

로그의 밑이 $$e$$인 경우 정보량 $$I(x)$$의 단위를 nats라고 한다.
밑이 2인 경우에는 bits 혹은 shannons라고 한다.

확률 분포에 대해서 정보량의 기댓값을 계산한 것을 Shannon entropy라고 한다.

$$H(\mathrm{x}) = \mathbb{E}_{\mathrm{x} \sim P} \left[ I(x) \right] = - \mathbb{E}_{\mathrm{x} \sim P} \left[ \log P(x) \right]. \tag{3.49}$$

$$\mathrm{x}$$가 연속적인 경우 Shannon entropy는 Differential entropy이다.

확률 변수 $$\mathrm{x}$$에 대해 두 분포 $$P(\mathrm{x})$$과 $$Q(\mathrm{x})$$가 있을 때,
두 분포의 차이를 Kullback-Leibler (KL) divergence를 통해 구할 수 있다.

$$D_{KL}(P \parallel Q) = \mathbb{E}_{\mathrm{x} \sim P} \left[ \log \frac{P(x)}{Q(x)} \right] = \mathbb{E}_{\mathrm{x} \sim P} \left[ \log P(x) - \log Q(x) \right]. \tag{3.50}$$

KL divergence는 0 이상의 값을 가진다. $$P$$와 $$ㅃ$$가 동일한 분포를 가질 때 0의 값을 가지고, 분포가 다를수록 값이 커진다.
종종 이를 두 분포의 거리처럼 생각할 수 있는데, $$D_{KL}(P \parallel Q) \ne D_{KL}(Q \parallel P)$$이기 때문에 거리 개념으로 사용할 수 없다.