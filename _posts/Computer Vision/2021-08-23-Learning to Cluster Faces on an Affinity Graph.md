...
근데 같은 그래프 형태에서 positive랑 negative가 반대로 뒤집힌 것이 학습으로 들어가도 잘 작동되는건가?

**Training and inference.**
위의 과정에서 positive와 negative vertex가 구분된 proposal들을 얻을 수 있다. 
이를 vertex 마다 BCE(Binary Cross Entropy) loss로 학습한다. 

추론 단계에서 하나의 proposal에서 여러 hypotheses를 얻어서 가장 좋은 것을 고른다고 하는데(threshold도 사용해서),
바로 위에 나온 여러 샘플 만드는 방식은 학습용 샘플 만드는 것인데 여러 가설 중에서 고른다는게 뭔소린지 모르겠다.
...
$$K=80$$으로 고정시키고 $$I, e_{\tau}, s_{max}$$를 다르게 해서 proposal 수를 다르게 해서 얻은 그래프라고 한다.
그런데 앞에 $$K$$가 나온 적이 없는 것 같은데 뭘 말하는지 모르겠다.
